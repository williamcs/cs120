# Databricks notebook source exported at Wed, 10 Aug 2016 12:05:53 UTC
# MAGIC %md
...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda (w, l): (w, sum(l)))
...
def removePunctuation(text):
    ...
    """
    return re.sub('[^a-z| |0-9]', '', text.strip().lower()) #re.sub(ur'[^\w\d\s]+', '', text.strip().lower()) #re.sub(r'[^\w\s]', '', text.strip().lower())  #ur'[^\w\d\s]+'
