# Databricks notebook source exported at Wed, 10 Aug 2016 12:08:49 UTC
# MAGIC %md
...
# TODO: Replace <FILL IN> with appropriate code
dic_len = len(sample_ohe_dict_manual.keys())
#sample_one_ohe_feat_manual = SparseVector(dic_len, [sample_ohe_dict_manual[(0, 'mouse')], sample_ohe_dict_manual[(1, 'black')]], [1., 1.])
sample_one_ohe_feat_manual = SparseVector(dic_len, {sample_ohe_dict_manual[(0, 'mouse')]: 1., sample_ohe_dict_manual[(1, 'black')]: 1.})
sample_two_ohe_feat_manual = SparseVector(dic_len, [sample_ohe_dict_manual[(0, 'cat')], sample_ohe_dict_manual[(1, 'tabby')], sample_ohe_dict_manual[(2, 'mouse')]], [1., 1., 1.])
sample_three_ohe_feat_manual = SparseVector(dic_len, [sample_ohe_dict_manual[(0, 'bear')], sample_ohe_dict_manual[(1, 'black')], sample_ohe_dict_manual[(2, 'salmon')]], [1., 1., 1.])
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    """
    return SparseVector(num_ohe_feats, {ohe_dict_broadcast.value[feature]: 1. for feature in raw_feats})
...
sample_distinct_feats_df = (sample_data_df
                            .select(explode(sample_data_df.features).alias('feature'))
                            .distinct())
...
# self define
sample_distinct_feats_df.rdd.map(lambda r: tuple(r[0])).zipWithIndex().collect()
...
def create_one_hot_dict(input_df):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    distinct_input_df = (input_df
                            .select(explode(input_df.features).alias('feature'))
                            .distinct())
    
    ohe_dict = (distinct_input_df
                   .rdd
                   .map(lambda r: tuple(r[0]))
                   .zipWithIndex()
                   .collectAsMap())
...
def parse_raw_df(raw_df):
    ...
    """
    return raw_df.select(split(raw_df.text, ',').getItem(0).alias('label').cast('double'), parse_point_udf(raw_df.text).alias('feature')).cache()
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    Returns:
        SparseVector: A SparseVector of length num_ohe_feats with indices equal to the unique
            ...
            with values equal to 1.0.
    """
    #return SparseVector(num_ohe_feats, { ohe_dict_broadcast[raw_feats[i]]: 1. for i in xrange(len(raw_feats)) if raw_feats[i] in ohe_dict_broadcast.value })
    #filter(lambda x: x in ohe_dict_broadcast.value, raw_feats)
    return SparseVector(num_ohe_feats, { ohe_dict_broadcast.value[raw_feats[i]]: 1. for i in xrange(len(raw_feats)) if raw_feats[i] in ohe_dict_broadcast.value })
...
# https://piazza.com/class/iqfbu516yuj5t3?cid=859     for reference
...
def add_log_loss(df):
    ...
    """
    #df_mod = df.withColumn('p_mod', when(df.p == 0.0, epsilon)
    #                                .when(df.p == 1.0, df.p - epsilon)
    #                                .otherwise(df.p))
    #return df_mod.withColumn('log_loss', when(...)).drop('p_mod')
  
    #return df.withColumn('log_loss', when(df.p == 0.0, when(df.label == 1, -0.0).otherwise(...))
    #                                 .when(df.p == 1.0, when(df.label == 0, -0.0).otherwise(...))
    #                                 .otherwise(...))
    return df.withColumn('log_loss', when((df.p == 0.0) & (df.label == 1), -log(lit(epsilon)))
                                     .when((df.p == 1.0) & (df.label == 0), -log(df.p - epsilon))
                                     .when(df.label == 1, -log(df.p))
                                     .otherwise(-log(1 - df.p)))
...
# self define
#ohe_train_df.groupBy().avg('label').show()
class_one_frac_train = (ohe_train_df.groupBy().avg('label')).collect()[0][0]
add_log_loss(ohe_train_df.withColumn('p', lit(class_one_frac_train)).select('p', 'label')).show()
add_log_loss(ohe_train_df.withColumn('p', lit(class_one_frac_train)).select('p', 'label')).groupBy().avg('log_loss').collect()[0][0]
...
log_loss_tr_base = (add_log_loss(ohe_train_df.withColumn('p', lit(class_one_frac_train)).select('p', 'label')).groupBy().avg('log_loss').collect()[0][0])
...
# self define
ohe_train_df.show()
add_probability(ohe_train_df, lr_model_basic).show()
...
def evaluate_results(df, model, baseline=None):
    ...
    #log_loss = with_log_loss_df.map(lambda x: x.log_loss).sum() / with_log_loss_df.count()
    log_loss = with_log_loss_df.select('log_loss').rdd.map(lambda r: r[0]).mean()
    return log_loss
...
hash_train_df.show()
#raw_train_df, raw_validation_df, raw_test_df
...
def vector_feature_sparsity(sparse_vector):
    ...
    """
    return len(sparse_vector.indices) /float(len(sparse_vector))
...
def get_sparsity(df):
    ...
    """
    #return (df.select('label', feature_sparsity_udf('features').alias('x')).groupBy().avg('x').collect()[0][0])
...
class_one_frac_test = (hash_test_df.groupBy().avg('label')).collect()[0][0]
print 'Class one fraction for test data: {0}'.format(class_one_frac_test)
log_loss_test_baseline = (add_log_loss(hash_test_df.withColumn('p', lit(class_one_frac_test)).select('p', 'label')).groupBy().avg('log_loss').collect()[0][0])
