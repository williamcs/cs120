# Databricks notebook source exported at Wed, 10 Aug 2016 12:07:06 UTC
# MAGIC %md
...
raw_data_df.select(sql_functions.split(raw_data_df.value, ',').alias('value')).map(lambda line: LabeledPoint(line[0][0], line[0][1:])).toDF().take(2)
...
def parse_point(df):
    ...
    """
    return df.select(sql_functions.split(df.value, ',').alias('value')).map(lambda line: LabeledPoint(line[0][0], line[0][1:])).toDF()
...
# self define
parsed_points_df.select(parsed_points_df.features, parsed_points_df.label-min_year).withColumnRenamed('(label - 1922.0)', 'label').show()
...
parsed_train_data_df.map(lambda lp: (average_train_year, lp.label)).take(2)
...
def gradient_summand(weights, lp):
    ...
    #return (dot(weights, lp.features)-lp.label)*lp.features
...
def linreg_gradient_descent(train_data, num_iters):
    ...
    for i in range(num_iters):
        ...
        gradient = train_data.map(lambda lp: gradient_summand(DenseVector(w), lp)).sum()
...
def two_way_interactions(lp):
    ...
    """
    new_features = [x[0] * x[1] for x in list(itertools.product(lp.features, lp.features))]
    feature = np.hstack((lp.features, new_features))
    return LabeledPoint(lp.label, feature)
